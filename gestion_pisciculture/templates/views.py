...
def ajouter_historique_sourcefine(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('liste_historique_sourcefine')  # Redirection vers la liste des poissons
    else:
        form = Historique_sourcefineForm()
    return render(request, 'historique_sourcefine_ajout.html', {'form': form})


def liste_historique_sourcefine(request):
    historique_sourcefine = Historique_Sourcefine.objects.all()
    return render(request, 'historique_sourcefine_liste.html', {'historique_sourcefine': historique_sourcefine})

def modifier_historique_sourcefine(request):
    id = request.GET.get('id')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('liste_historique_sourcefine')  # Redirection après modification
    else:
...
def supprimer_historique_sourcefine(request):
    historique_sourcefine_id = request.Get.get('id')
    historique_sourcefine = get_object_or_404(Historique_Sourcefine, id=historique_sourcefine_id)
    if request.method == 'POST':
        historique_sourcefine.delete()
        return redirect('liste_historique_sourcefine')  # Redirection après suppression
